...
async function main() {
  ...
  const tx = await router.claimRefundAndRewards(eventId, tierId);
  await tx.wait();
  console.log("Claimed refund and rewards:", { txHash: tx.hash, eventId: eventId.toString(), tierId: tierId.toString() });
}
